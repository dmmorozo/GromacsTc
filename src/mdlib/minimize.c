...
//    gmx_fatal(FARGS,"Cannot do parallel L-BFGS Minimization - yet.\n");
  ...
  where();
  for(i=0;i<n;i++)
    if(frozen[i]) ff[i]=0.0;  /* Apply frozen groups */
  ...
  for(step=0; (number_steps<0 || (number_steps>=0 && step<=number_steps)) && !converged; step++) {
	...
	//Call line search
	ls = line_search_morethuente(n, xx, &Epot, ff, s, frozen, &stepsize, lastx, lastf, ems, fplog, cr, gstat, top, enerd, graph, vir, pres, mu_tot, step, bVerbose, vsite, constr, inputrec, top_global, fcd, state, mdatoms, nrnb, wcycle, fr, &bReset);
	 ...
	 * If not, reset memory to restart as steepest descent before quitting.
		...
		break;
			...
			/* Set Flag */
			bReset=TRUE;
			fprintf(stderr,"LBFGS reset: Linesearch=%d, stepsize=%.12lf, fnorm=%.12lf\n",ls,stepsize,fnorm);
			continue;
		}
	}
	else {
		if (ls>1) fprintf(stderr, "Linesearch converged in %d steps\n",ls-1);
			//fprintf(stderr, "Linesearch converged in %d steps: stepsize=%.12lf, Energy=%.12lf, Ed=%.12lf\n", ls, stepsize, Epot, Epot-Epot0);
	}
    ...
    if (MASTER(cr)) {
//      if(bVerbose)
	fprintf(stderr,"Step %6d, Epot= %.8lf, Fnorm= %.8lf, Fmax= %.8lf (atom %d), stepsize= %.8lf, deltaE= %.8lf\n",
		step,Epot,fnorm/sqrt(state->natoms),fmax,nfmax+1,stepsize, Epot-Epot0);
      /* Store the new (lower) energies */
      upd_mdebin(mdebin,FALSE,FALSE,(double)step,
		 mdatoms->tmass,enerd,state,state->box,
		 NULL,NULL,vir,pres,NULL,mu_tot,constr);
...
)
{
	...
	real ftol = 1.0E-4;
	real gtol = 0.9;
	real min_step = 1.0E-20, max_step=1.0E+20;
	int max_linesearch = 20;
	real xtol = GMX_REAL_EPS;
	...
	for (dginit = 0, i = 0; i < n; i++) {
		dginit -= s[i] * g[i];   
	}
	//fprintf(stderr, "s0=%.12lf g0=%.12lf dginit=%.12lf\n", s[0], g[0], dginit);
...
//		return -2;
	...
	*/
	...
	for (;;) {
		 ...
		 */
		if ((brackt && ((*stp <= stmin || stmax <= *stp) || max_linesearch <= count + 1 || uinfo != 0)) || (brackt && (stmax - stmin <= xtol * stmax))) {
			*stp = stx;
		}
		/*
		 ...
		 */
                for(i=0;i<n;i++)
    		  if (frozen[i]) s[i]=0.0;  /* Apply frozen groups */
		...
		evaluate_energy(fplog, bVerbose, cr,
			...
			inputrec, nrnb, wcycle, gstat,
			vsite, constr, fcd, graph, mdatoms, fr,
			mu_tot, enerd, vir, pres, step, *bReset);
		if (*bReset) *bReset=FALSE;

		*f = ems.epot;
                
                for(i=0;i<n;i++)
    		  if (frozen[i]) g[i]=0.0;  /* Apply frozen groups */

		//for (i = 0; i < n; i++)
		//	g[i] = -g[i];  /* make gradient */

		for (dg = 0, i = 0; i < n; i++) {
			dg -= s[i] * g[i];
		}

		//fprintf(stderr, "count= %d stp= %.12lf E= %.12lf dg=%.12lf \n", *stp, count, *f, dg);
		...
		/*
			...
			function has a nonposi11tive function value and nonnegative
...
)
{
	...
	//fprintf(stderr, "stx=%.12lf fx=%.12lf dx=%.12lf\n", *x, *fx, *dx);
	//fprintf(stderr, "sty=%.12lf fy=%.12lf dy=%.12lf\n", *y, *fy, *dy);
	//fprintf(stderr, "stp=%.12lf ft=%.12lf dt=%.12lf\n", *t, *ft, *dt);
	//fprintf(stderr, "tmin=%.12lf tmax=%.12lf brackt=%d\n", tmin, tmax, *brackt);
	/*
