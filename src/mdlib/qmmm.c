 ...
 */
void
init_msdsh_tc(t_commrec *cr ,t_QMrec *qm, t_MMrec *mm);

real
call_msdsh_tc(t_commrec *cr,t_forcerec *fr, t_QMrec *qm,
		...
		t_MMrec *mm, rvec f[], rvec fshift[])
{
	...
	QMener = call_msdsh_tc(cr, fr, qm, mm, f, fshift);
	return (QMener);
}
		...
		gmx_mtop_t *mtop, t_inputrec *ir)
{
	...
	*/
	...
	qm->nstates        = ir->opts.nstates[grpnr];
	qm->currstate      = ir->opts.currstate[grpnr];
...
t_QMrec *copy_QMrec(t_QMrec *qm)
{
	...
	qmcopy->bOPT         = qm->bOPT;
	qmcopy->nstates      = qm->nstates;
	qmcopy->currstate    = qm->currstate;
	qmcopy->delta_t      = qm->delta_t;
		...
		t_forcerec *fr)
{
	...
	if(qr->nrQMlayers==1){
		...
		init_msdsh_tc(cr, qr->qm[0], qr->mm);

		/* output the qm specific stuff
		 * file is not closed when gmx finishes
		 * might cause problems
		 * the gmx old-file-backup mechanism
		 * does not work here!
		 */
		qr->QMoutput = fopen( "qm.log", "w");
	}
} /* init_QMMMrec */
