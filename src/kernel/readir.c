...
static char QMmethod[STRLEN],QMbasis[STRLEN],QMcharge[STRLEN],QMmult[STRLEN],
  ...
  nstates[STRLEN],currstate[STRLEN];
            ...
            warninp_t wi)
{
  ...
  STYPE ("bTS",          bTS, NULL);
  CTYPE ("Number of states in CI");
  STYPE ("nstates",      nstates, NULL);
  CTYPE ("Starting state in first step");
  STYPE ("startstate",   currstate, NULL);
              ...
              warninp_t wi)
{
  ...
  int     nQMmethod,nQMbasis,nQMcharge,nQMmult,nbSH,nCASorb,nCASelec,
    nSAon,nSAoff,nSAsteps,nQMg,nbOPT,nbTS,nnstates,ncurrstate;
  char    warn_buf[STRLEN];
  ...
  nnstates   = str_nelem(nstates,MAXPTR,ptr1);
  ncurrstate = str_nelem(currstate,MAXPTR,ptr2);
  snew(ir->opts.nstates,nr);
  snew(ir->opts.currstate,nr);
  for(i=0;i<nr;i++)
  {
	  ir->opts.nstates[i]   = strtol(ptr1[i],NULL,10);
	  ir->opts.currstate[i] = strtol(ptr2[i],NULL,10);
  }
  /* end of QMMM input */
